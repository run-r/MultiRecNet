...
class Channel_Embeddings(nn.Module):
    ...
    def __init__(self, patchsize, img_size, in_channels):
        ...
        # n_patches = np.prod([im_d // p_d for im_d, p_d in zip(img_size, patch_size)])
...
class Modality_2_Attention(nn.Module):
    def __init__(self, vis ,channel_num):
        ...
        self.psi = nn.InstanceNorm3d(self.num_attention_heads)
...
class Attention_org(nn.Module):
    def __init__(self, vis ,channel_num,dim):
        ...
        for _ in range(4):
            ...
            key = nn.Linear( dim*4,  dim*4, bias=False)
            value = nn.Linear(dim*4,  dim*4, bias=False)
            self.query1.append(copy.deepcopy(query1))
            self.query2.append(copy.deepcopy(query2))
    ...
    def forward(self, emb1,emb2,emb3, emb4,emb_all):
        ...
        multi_head_Q4 = torch.stack(multi_head_Q4_list, dim=1) if emb3 is not None else None
        ...
        multi_head_Q4 = multi_head_Q4.transpose(-1, -2) if emb3 is not None else None
        ...
        attention_scores4 = torch.matmul(multi_head_Q4, multi_head_K) / math.sqrt(self.KV_size) if emb4 is not None else None
        ...
        attention_probs4 = self.attn_dropout(attention_probs4) if emb3 is not None else None
        ...
        context_layer4 = torch.matmul(attention_probs4, multi_head_V) if emb3 is not None else None
        ...
        context_layer4 = context_layer4.permute(0, 3, 2, 1).contiguous() if emb3 is not None else None
        ...
        context_layer4 = context_layer4.mean(dim=3) if emb3 is not None else None


        O1 = self.out1(context_layer1) if emb1 is not None else None
        O2 = self.out2(context_layer2) if emb2 is not None else None
        O3 = self.out3(context_layer3) if emb3 is not None else None
        O4 = self.out4(context_layer4) if emb3 is not None else None

        O1 = self.proj_dropout(O1) if emb1 is not None else None
        O2 = self.proj_dropout(O2) if emb2 is not None else None
        O3 = self.proj_dropout(O3) if emb3 is not None else None
        O4 = self.proj_dropout(O4) if emb3 is not None else None
...
class Block_ViT(nn.Module):
    def __init__(self, vis, channel_num,dim):
        ...
        self.attn_norm =  LayerNorm(channel_num[2]*4,eps=1e-6)
...
class Encoder(nn.Module):
    ...
    def forward(self, emb1,emb2,emb3,emb4):
        ...
        emb4 = self.encoder_norm4(emb4) if emb3 is not None else None
...
class ChannelTransformer2(nn.Module):
    def __init__(self, vis, img_size, dim,channel_num=[64, 128, 256], patchSize=[32, 16, 8],):
        ...
        self.embeddings_4 = Channel_Embeddings(self.patchSize_4, img_size=img_size, in_channels=channel_num[3])
    ...
    def forward(self,en1,en2,en3,en4):
        ...
        emb1, emb2, emb3,emb4, attn_weights = self.encoder(emb1,emb2,emb3,emb4)
